...
namespace Servicio.Acceso
{
    public class Startup : BaseBuilder
    {
        ...
        public System.IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddScoped<IAccesosRepository, MsSqlAccesos>();
            services.AddScoped<ISolicitarAccesoRepository, MsSqlSolicitarAcceso>();
            services.AddScoped<ICodigoGeneradoValidacionRepository, MsSqlCodigoGeneradoValidacion>();
            services.AddSingleton<ServicioMessage>();
            services.AddSwaggerGen(c =>
            {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "Servicio.Acceso", Version = "v1" });
            });
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory, Microsoft.AspNetCore.Hosting.IApplicationLifetime lifetime)
        {
            if (env.IsDevelopment())
            {
                ...
                app.UseSwagger();
                app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "Servicio.Acceso v1"));
            }
